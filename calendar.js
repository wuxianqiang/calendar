...
var Animals = new Array("鼠年", "牛年", "虎年", "兔年", "龙年", "蛇年", "马年", "羊年", "猴年", "鸡年", "狗年", "猪年");
    ...
    for (i = 0; i < 42; i++) {
        ...
        if (sD > -1 && sD < cld.length) { //日期内
            ...
            if (cld[sD].isToday) {
                ...
                sObj.style.fontWeight = '700';
                sObj.style.textShadow = '0px 2px 10px #e0e0e0';
            } //今日颜色
            else {
                sObj.style.color = '';
            }
            ...
            if (s.length <= 0) { //设置节气的颜色
                s = cld[sD].solarTerms;
                if (s.length > 0) s = s.fontcolor('#8fad8f');
            }
            if (s.length > 0) {
                lObj.innerHTML = s;
                Slfw = s;
